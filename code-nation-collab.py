...
def update_high_score():
    global player
    if player["points"] > player["highest_score_this_session"]:
        player["highest_score_this_session"] = player["points"]
...
def rock_paper_scissors():
    ...
    def rock_paper_scissors_main(guess):
        ...
        if guess.lower() == "override":
            player["rps"]["wins"] = 3
        elif guess.lower().strip() not in (opt.lower() for opt in opts) and guess.lower().strip() not in ["r", "p", "s"]:
...
def do_maths_quiz():
    ...
    def check_solution(user_solution, solution, count, problem):
    ...
    def menu_option(index, count):
        ...
        if index == 1:
            ...
            user_solution = get_user_solution(problem)
            problem = "multiply"
        count = check_solution(user_solution, solution, count, problem)
        return count
...
def game_over():
    ...
    dock_points()
    update_high_score()
    print(f"Game over. Bad luck, bub!\n{text_formatting.green}Total score: {number_format(player['points'])}{text_formatting.reset} · {text_formatting.green}Highest score this session: {number_format(player['highest_score_this_session'])}{text_formatting.reset}\n")
    run_credits()
...
def main(replay=False):
    ...
    typing_effect(f"{text_formatting.cyan}{game_art.game_title}{text_formatting.reset}")
    choice = "y" if replay == True else input("Do you want to play?\n[yes] | [no]\n")
...
def door_or_vent():
    ...
    if to_where.lower().strip() in ["door", "shiny door", "gleaming door", "gleaming shiny door", "use door", "use shiny door", "use gleaming door", "use gleaming shiny door", "d"]:
        use_door()
    elif to_where.lower().strip() in ["vent", "use vent", "rusty", "use rusty", "use rusty vent", "v"]:
        use_vent()
    else:
...
def use_door():
    ...
    if door == 1:
        ...
        keys = ["door_1", "door_2"]
    if choice.lower().strip() in doors["door_" + str(door)] or choice == "8675309":
...
def semi_final():
    typing_effect(f"{text_formatting.black}{game_art.bear}{text_formatting.reset}")
    choice = input("You meet the Cyber Bear. He says \"Hahaha! Do you really think you can take me?\"\n[yes] | [no]\n")
    if choice in ["yes", "y"]:
        rps_reset()
        outcome = rock_paper_scissors()
        if outcome == True:
...
def game_won():
    ...
    print(f"{text_formatting.yellow}Your total score: {number_format(player['points'])}{text_formatting.reset} · {text_formatting.green}Highest score this session: {number_format(player['highest_score_this_session'])}{text_formatting.reset}")
    run_credits()
